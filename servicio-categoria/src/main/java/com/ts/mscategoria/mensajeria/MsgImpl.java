...
public class MsgImpl implements Msg {
  ...
  /**
   * Proceso de mensajeria encargado de CREAR, ACTUALIZAR y ELIMINAR categorias. (Suscripcion)
   */
  @Override
  public void procesarCUD() {
    ...
    try {
      ...
      LOGGER.info(
          "[*] Esperando por solicitudes de creacion de categorias. Para salir presiona CTRL+C");
      ...
      DeliverCallback deliverCallback = (consumerTag, delivery) -> {
        ...
        switch (delivery.getEnvelope().getRoutingKey()) {
          case ROUTE_KEY_CREATE: {
            ...
            LOGGER.info(
                "[x] Recibido por queue '" + receiverQueue + "' -> " + categoriaRoot.toString());
  ...
  /**
   * Proceso de mensajeria encargado de enviar listado de categorias (Request-Response Sincronico a
   * solicitudes desde Apigateway y MsNoticia)
   */
  @Override
  ...
  /**
   * Funcion utilizada para enviar el listado de categorías a los miscroservicios que esten
   * suscritos a la actualizacion de categorias (Publicar)
   */
  private void enviarListaCategorias() {
    ...
    try {
      ...
      LOGGER.info(
          "[x] Enviando por exchange '" + EXCHANGE_NAME + "' por ruta '" + ROUTE_KEY_LIST_SUBS
              + "' ->" + mapList);
